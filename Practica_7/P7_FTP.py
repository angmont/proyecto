...
def busqueda(hola):
  lista = []
  for i in hola:
    exp = re.compile(r".+\.(txt|msg|README)$")
    busqueda = exp.search(i)
    if (busqueda) != None:
      lista.append(busqueda.group())
  return lista
...
def connect_ftp(
    ...
    for dir_info in l_dir:
        ...
        for dir2_info in l_subdir:
		        file_name = get_file_name(dir2_info)        
		        lista.append(file_name)
    pepe = busqueda(lista)
    print(lista)
    #print(pepe)
    for i in pepe:
      #print(i)
      try:
          save_file(connection, f"{actual_path}/{i}", f"{save_path}/{i}")
      except:
          #print("error")
          continue
...
if __name__ == "__main__": 
  #connect_ftp(host="ftp.mirror.nl", save_path="C:/Users/amont/Escritorio/proyecto/Practica_7/TXT")
  connect_ftp(host="ftp.rediris.es", save_path="C:/Users/amont/Escritorio/proyecto/Practica_7/TXT")
